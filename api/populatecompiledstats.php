...
$conn->query("DELETE FROM compiledstats WHERE player > 0");

$get_players_sql = "SELECT * FROM players";

foreach($conn->query($get_players_sql) as $p) {
	$player = $p["id"];
	$tourney_games_sql = "SELECT * FROM game WHERE (homeOne='$player' OR homeTwo='$player' OR awayOne='$player' OR awayTwo='$player')";
	...
	$insert_sql = "INSERT INTO compiledstats (player, makes, shots, ginobs, madeginobs, voms, difectas, trifectas, ballsback, forcedOTs, games, redemptionmakes, redemptionshots) VALUES ('$player', " . $makes . ", " . $shots . ", " . $ginobs . ", " . $madeginobs . ", " . $voms . ", " . $difectas . ", " . $trifectas . ", " . $ballsback . ", " . $forcedOTs . ", " . $games . ", " . $redemptionMakes . ", " . $redemptionShots . ")";
