...
$tourney_games_sql = "SELECT * FROM game WHERE tournament='Y'";
$comp = array();

foreach($conn->query($tourney_games_sql) as $game) {
	$id = $game["id"];
	$sql = "SELECT * FROM statline WHERE game='$id'";
...
foreach($comp as $player) {
	$sql = "UPDATE tourneystats SET makes=" . $player["makes"] . ", shots=" . $player["shots"] . ", ginobs=" . $player["ginobs"] . ", madeginobs=" . $player["madeginobs"] . ", voms=" . $player["voms"] . ", ballsback=" . $player["ballsback"] . ", trifectas=" . $player["trifectas"] . ", difectas=" . $player["difectas"] . ", forcedOTs=" . $player["forcedOTs"] . ", games=" . $player["games"] . ", redemptionshots=" . $player["redemptionshots"] . ", redemptionmakes=" . $player["redemptionmakes"] . " WHERE player=" . $player["player"];
	$conn->query($sql);
}
